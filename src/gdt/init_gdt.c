...
#define SEG_CODE_EXRD (0b100 | 0b010)            // Execute/Read
#define SEG_DATA_RDWR 0b010            // Read/Write
...
static void create_segment(struct gdt_segment *entry, unsigned int limit, unsigned int base, unsigned char access, unsigned char flags)
{
    ...
    entry->access_byte = access & 0xF;
    entry->flags = flags & 0xF;
}

void init_gdt(void)
{    
    struct gdt_segment *gdt_entries = (struct gdt_segment *)GDT_ADDRESS;
    struct gdt_ptr gdt;

    gdt.addr = GDT_ADDRESS;
